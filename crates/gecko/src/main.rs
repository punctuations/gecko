...
fn run_code(code: &bytecode::Code) -> Result<String, Failure> {
    ...
    let run = vm.run(code);
    if run.error {
        let message = if run.message.is_empty() { "RuntimeError".into() } else { run.message };
        return Err(Failure {
            output: run.output,
            message,
        });
...
mod tests {
    ...
    #[test]
    fn hello_world() {
        assert_eq!(run_source("print(\"hello world\")\n").unwrap(), "hello world\n");
    }
    ...
    fn short_circuit_and_or() {
        assert_eq!(run_source("print(1 and 2)\nprint(0 or 5)\n").unwrap(), "2\n5\n");
    }
    ...
    fn floats_print_shortest_repr() {
        let src = "print(100.0)\nprint(2.5)\nprint(340.0 / 9.0)\nprint(0.0001)\nprint(1e16)\n";
        assert_eq!(run_source(src).unwrap(), "100.0\n2.5\n37.77777777777778\n0.0001\n1e+16\n");
    }
    ...
    fn dict_literals_and_methods() {
        ...
        assert_eq!(run_source(src).unwrap(), "{'a': 1, 'b': 2} 2 2 9\n['a', 'b'] [1, 2]\n");
    }
    ...
    fn full_signature_binds_correctly() {
        ...
        assert_eq!(run_source(src).unwrap(), "(1, 10, (), {})\n(1, 2, (3, 4), {'p': 5})\n");
    }
    ...
    fn too_many_positionals_without_varargs_errors() {
        ...
        assert!(f.message.contains("positional arguments but 3"), "{}", f.message);
    }

    #[test]
    fn unexpected_keyword_argument_errors() {
        let f = run_source("def f(a):\n    return a\nf(x=1)\n").unwrap_err();
        assert!(f.message.contains("unexpected keyword argument"), "{}", f.message);
    }

    #[test]
    fn duplicate_argument_value_errors() {
        let f = run_source("def f(a, b):\n    return a\nf(1, a=2)\n").unwrap_err();
        assert!(f.message.contains("multiple values for argument"), "{}", f.message);
    }
    ...
    fn string_constants_fold() {
        assert_eq!(run_source("print(\"a\" + \"b\" + \"c\")\n").unwrap(), "abc\n");
    }
    ...
    fn finally_runs_on_both_paths() {
        let src =
            "try:\n    print(\"ok\")\nfinally:\n    print(\"cleanup\")\ntry:\n    try:\n        raise TypeError(\"x\")\n    finally:\n        print(\"inner cleanup\")\nexcept TypeError:\n    print(\"outer\")\n";
        assert_eq!(run_source(src).unwrap(), "ok\ncleanup\ninner cleanup\nouter\n");
    }
    ...
    fn tuples_pack_unpack_and_compare() {
        ...
        assert_eq!(run_source(src).unwrap(), "(1, 'two') two 1 1 2 3\nTrue (1, 2, 3) 0 True\n");
    }
    ...
    fn nested_break_binds_the_inner_loop() {
        let src =
            "hits = []\nfor a in range(3):\n    for b in range(9):\n        if b > a:\n            break\n        hits.append((a, b))\nprint(hits)\n";
        assert_eq!(run_source(src).unwrap(), "[(0, 0), (1, 0), (1, 1), (2, 0), (2, 1), (2, 2)]\n");
    }
    ...
    fn comprehensions_close_over_enclosing_scopes() {
        ...
        assert_eq!(run_source(src).unwrap(), "[0, 3, 6, 9]\n[[], [1], [1, 2]]\n[9, 9]\n");
    }
    ...
    fn garbage_stays_bounded() {
        ...
        assert!(vm.heap_live() < 5000, "heap has {} live objects", vm.heap_live());
    }
